...
Copyright (c) 2007 Dymchenko Artem. All rights reserved.

Created 20. 6. 2006 by Dymchenko Artem and Tamara Nikolaevna
Last modified: 28. 3. 2007

Cross-browser JavaScript function grapher.

Построитель распростряняется бесплатно;
Вы можете модифицировать/изменять содержимое данного покета, согласно GNU General Public License.
Это приложение распространяется с целью частичного ознакомления с основными возможностями 
Построителя графиков функций, являющимся научным проектом ученика СОШ №35 города Экибастуза Дымченко Артёма.
С целью дополнительного ознакомления с GNU General Public License посетите официальный сайт http://www.gnu.org/copyleft/gpl.html
...
var GRAPHCOLOR = ['ff0000', '009f00', '4444ff', 'ffaa00', '00e0e0',	'b00099', '00d000', 'cc6633', '3333cc', 'ff7744', '00c0aa', 'ff00ff', '006f00', 'cc0000', '0096d0', '9900ee'];
var DEFW = screen.width-220; // стандартная ширина области построения
var LEFT = 50;  // y-отступ от верхнего края
var TOP = 4;    // верх    "
var RIGHT = 124;// right "  дополнительные опции
var BOTTOM = 22;// bottom для отступа в x сетке
/*********   End config variables   ***********/

var g_frame, g_inputTb, g_cnv,         // область построения
g_gridlyr, g_xlyr, g_ylyr,// слои для x и y
g_termlyr,     // слои серий
g_optslyr, g_coordtip,    //подсказки координат
g_n = 0,       //число в подсказке
g_htm = '', g_term, g_w, g_h,      // размер подсказки
g_xa, g_xz,    // x min, x max
g_ya, g_yz, g_dx, g_dy,    // значение дельта в пикселях
g_pageX, g_pageY, // позиция указателя
g_wait = 0, g_infin = 100000000000000000000, g_adhtm, g_dom, g_moz, g_ua = navigator.userAgent.toLowerCase(),
...
g_dom = (!g_adhtm &&typeof g_frame.appendChild != "undefined" &&	typeof document.createRange != "undefined" &&typeof (o = document.createRange()).setStartBefore != "undefined" &&typeof o.createContextualFragment != "undefined");
g_moz = document.body && typeof document.body.style.MozOpacity != "undefined";
getCnvPos();
paint(g_frame,mkDiv('', '', LEFT, TOP, g_w+1, g_h+1, CNVCOLOR, 'bglyr')+mkDiv('', '', LEFT, TOP, g_w+1, g_h+1, '', 'gridlyr')+mkDiv('', '', LEFT, TOP, g_w+1, g_h+1, '', 'cnvlyr')+mkDiv('', '', LEFT, TOP+g_h+1, g_w+1, BOTTOM, '', 'xlyr', true)+mkDiv('', 'right', 0, TOP, LEFT, g_h+1, '', 'ylyr', true)+mkDiv('', '', LEFT+g_w+4, TOP+(g_h>>1)-200, RIGHT, g_h, '', 'opts', true));

paint(getDiv('opts'),	'<input name="GRID" type="checkbox" checked onclick="setTimeout(\'setGridVis();\', 10)">'+S_SHOWGRID+'<br>&nbsp;<br><input name="XYRATIO" type="checkbox">'+S_SAMESCALE+'<br>&nbsp;<br><input name="FITWND" type="checkbox">'+S_FITWND+'<br>&nbsp;<br><input name="PRT" type="checkbox" onclick="setTimeout(\'prt();\', 10)">'+S_PRTVIEW+	'<br>&nbsp;<br>&nbsp;<br>&nbsp;<br>'+S_POSTREPING,true);
...
function initCoordtip(o){(o = (g_coordtip = getDiv('coordtip')).style).left = (g_pageX+15)+'px';
o.top = (g_pageY+10)+'px';
o.zIndex = 2;
...
function prt()
{getDiv('body1').style.display = getDiv('body2').style.display = (document.forms.fo.PRT.checked)? 'none' : 'block';getCnvPos();}
...
x = ''+Math.abs(x);
	while (n--)
{if (x.charAt(x.length-1) == '0')	x = x.substring(0, x.length-1);
else if (!(/\./).test(x))
x = x.substring(0, x.length-1)+'.'+x.charAt(x.length-1);
else if (!x.indexOf('.')) x = '.0'+x.substring(1);
...
htm = '', gridhtm = '',
pos, str, strs, strl,
limt = Math.round(g_xz/g_dx), limb = Math.round(g_xa/g_dx), j = Math.ceil(g_xa/ivx), i = Math.round(j*rx);
while (i-limt <= 0)
{strs = (str = ''+mkNumStr(j, ivx, true)).match(/[^\>\<a-z]+/g);
strl = strs[0].length + (strs[1]? strs[1].length : 0);
htm += mkDiv('', '', (pos = i-limb), 0, 1, 4, SCALECOLOR, '', false, true)+mkDiv('<font style="font-size:11px;font-family:Arial,Helvetica,sans-serif;color:black;">'+str+'<\/font>', '', pos-3*strl, 4, 16*strl, 12, '', '', true, false);
gridhtm += mkDiv('', '',  pos, 0, 1, g_h+1, !j? AXCOLOR : GRIDCOLOR, '', false, true);
i = Math.round(++j*rx);}
paint(g_xlyr, htm);
var sclw = parseInt(g_ylyr.style.width);
htm = ''; limt = Math.round(g_yz/g_dy), limb = Math.round(g_ya/g_dy);
j = Math.ceil(g_ya/ivy);
i = Math.round(j*ry);
while (i-limt <= 0)
{strl = (str = ''+mkNumStr(j, ivy, true)).length;
htm += mkDiv('', '', sclw-4, (pos = g_h-i+limb), 4, 1, SCALECOLOR, '', false, true)+mkDiv('<font style="font-size:11px;font-family:Arial,Helvetica,sans-serif;color:black;">'+str+'<\/font>', 'right', 0, pos-7, sclw-6, 12, '', '', true, false);
...
return ('<div'+(id? ' id="'+id+'"' : '')+' style="position:absolute;left:' + x + 'px;top:' + y + 'px;width:' + w + 'px;height:' + h + 'px;'+(over? '' : ('clip:rect(0,' + w + 'px,' + h + 'px,0);' + (g_moz? '' : 'overflow:hidden;')))+(c? 'background-color:' + c + ';' : '')+'text-align:' + (align || 'left') + ';">' + txt + '<\/div>');}

function gReplace(x)
{return x.replace(/%%([^;]+);([^;]+);([^;]+);([^;]+);([^;]+);([^;]*);([^;]+);([^%%]*)/g,	'<div style="position:absolute;left:$1px;top:$2px;width:$3px;height:$4px;text-align:$5t;'+'background:$6;overflow:$7;"$8<\/div>');}
...
function mkTerm(x){x = x.toLowerCase().replace(/\s/g,'').replace(/f(\(|\{|\[)x(\)|\}|\])\=/g, '').replace(/math\./g,'').replace(/\.*\;+/g,';').replace(/[\.\;]*$/,'').replace(/\|([^\|]*)\|/g,'abs($1)').replace(/:/g,'\/').replace(/І/g,'^2').replace(/і/g,'^3').replace(/[\"\']/g,'^').replace(/e/g,'E').replace(/log(\d)/g,'LOG$1').replace(/ln(\d)/g,'LN$1').replace(/arc(us)?|arEa|ar/g,'a').replace(/hyp(Erbolicus)?/g,'h').replace(/cosin(us|E)?/g,'cos').replace(/sin(us|E)/g,'sin').replace(/sEc(an(s|t))?/g,'sec').	replace(/cosec/g,'csc').replace(/tangEn[st]/g,'tan').replace(/cotan/g,'cot').replace(/random(\(|\{|\[)(\)|\}|\])|rand(om)?/g,'random()').replace(/(p|power)(\(|\{|\[)/g,'pow$2').replace(/(b|q)r(\(|\{|\[)/g,'$1rt$2').replace(/(sqr|sqrt)(\d)/g,'SQRT$2').replace(/tan2/g, 'tanz').replace(/max/g,'maz').replace(/pi/g,'PI').replace(/phi/g,'PHI').replace(/y/,'nuts').replace(/cEil/g,'ceil').replace(/Expow|Exp/g,'exp').replace(/([^m]|^)(\(|\{|\[)(\)|\}|\])/g,'$1$2x$3').replace(/(\(|\{|\[)\,/g,'$1x,').replace(/\,(\)|\}|\])/g,',x$1').replace(/\,\,/g,',x,');
...
this.fact = function(x){x = Math.round(Math.abs(x));
...
while (/\^/.test(x))
{var az = x.search(/\^/), a = x.substring(0, az), z = x.substring(az+1), p = 0, ch, lim = az-1;
while (true)
{ch = a.charAt(lim);
if (ch == ')') p++;
else if (ch == '(') p--;
...
{var yy = parseFloat(this), y, incr = g_dx/100, _incr = incr*xdir, qt = 1, lim = (g_xa+(j+xdir)*g_dx), x = g_xa+j*g_dx;
...
return;}
g_htm = '';
var xa = getVal(f.XA), xz = getVal(f.XZ);
...
{if (yy[i][j-1] < 0 && y > 0 && yy[i][j+1]-y < 0) y = y.lim(i,j,-1,1);
else if (yy[i][j-1] > 0 && y < 0 && yy[i][j+1]-y > 0) y = y.lim(i,j,-1,-1);
else if (yy[i][j+1] > 0 && y < 0 && yy[i][j-1]-y > 0) y = y.lim(i,j,1,-1);
else if (yy[i][j+1] < 0 && y > 0 && yy[i][j-1]-y < 0) y = y.lim(i,j,1,1);}
else if (j-g_w && !isNum(yy[i][j+1]) && isNum(yy[i][j-1])) y = y.lim(i,j,1,(y-yy[i][j-1] > 0)? 1 : -1);
else if (j && !isNum(yy[i][j-1]) && isNum(yy[i][j+1])) y = y.lim(i,j,-1,(y-yy[i][j+1] > 0)? 1 : -1);
yy[i][j] = y;}
if (!isNum(g_ya)) ya = Math.min(ya, y);
if (!isNum(g_yz)) yz = Math.max(yz, y);}}}}catch (err){alertErr(f.F, S_ERR_TERM);
return;}
if (!g_n)
{g_yz = yz;
g_ya = ya;
...
var x = (e = e || window.event).pageX || e.clientX || 0, y = e.pageY || e.clientY || 0; if (g_ie || g_kq)
...
paint(g_coordtip, '<small style="background:'+CNVCOLOR+';white-space:nowrap;">x = '+ mkNumStr(x-g_pageX+Math.round(g_xa/g_dx), g_dx)+'&nbsp;<br>y = '+	mkNumStr(g_h-y+g_pageY+Math.round(g_ya/g_dy), g_dy)+'&nbsp;<\/small>', false);
